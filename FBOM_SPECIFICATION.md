...
Attack surface analysis clusters grouping all reachable functions within each dependency package, along with entry points from user code.

```json
{
  "name": "string",                    // Dependency package name
  "entry_points": [ ... ],             // Entry points from user code to this dependency
  "cluster_functions": ["string"],     // All reachable functions within this dependency
  "total_blast_radius": "number"       // Total count of cluster_functions
}
```
...
Dependency clustering provides rapid security assessment capabilities:

- **Attack Surface Mapping**: Identifies all functions that could be affected by a vulnerability in a specific dependency
- **Blast Radius Analysis**: Quantifies the potential impact scope with the `total_blast_radius` count
- **Entry Point Tracking**: Shows exactly how your code interacts with each dependency
- **CVE Impact Assessment**: Enables quick evaluation of whether a CVE affects reachable code paths
- **Standardized Analysis**: Provides consistent dependency risk evaluation across projects
