...
// DependencyCluster represents a cluster of functions within a dependency
...
// DependencyEntry represents an entry point into a dependency
...
type AnalysisContext struct {
	...
	EntryPoints        []string
	DependencyClusters []DependencyCluster // New field for cluster-based reachability
}
...
func (e *Engine) AnalyzeCVE(ctx *AnalysisContext, targetCVE cve.CVE) (*Assessment, error) {
	...
	if len(ctx.DependencyClusters) > 0 {
		reachability = e.analyzeReachabilityWithClusters(ctx, vulnerableNodes, targetCVE)
	} else {
...
// analyzeReachabilityWithClusters uses dependency clusters for faster reachability analysis
func (e *Engine) analyzeReachabilityWithClusters(ctx *AnalysisContext, vulnerableNodes []*callgraph.Node, targetCVE cve.CVE) ReachabilityResult {
	result := ReachabilityResult{
		Status:      NotReachable,
		MinDistance: -1,
	}
	...
	if e.verbose {
		fmt.Printf("Using cluster-based reachability analysis for CVE %s\n", targetCVE.ID)
	}
	...
	for _, vulnNode := range vulnerableNodes {
		...
		if e.verbose {
			fmt.Printf("DEBUG: Checking reachability for vulnerable function: %s in package: %s\n", vulnFuncName, vulnPackage)
		}

		// Check each dependency cluster
		for _, cluster := range ctx.DependencyClusters {
			...
			if e.verbose {
				fmt.Printf("DEBUG: Checking cluster %s (functions: %d, entry_points: %d)\n",
					cluster.Name, len(cluster.ClusterFunctions), len(cluster.EntryPoints))
			}
			...
			if len(cluster.EntryPoints) > 0 {
				if e.verbose {
					fmt.Printf("DEBUG: REACHABLE! Function %s found in cluster %s with %d entry points\n",
						vulnFuncName, cluster.Name, len(cluster.EntryPoints))
				}

				// Create a call path showing reachability via cluster
				...
				// Mark as reachable
				result.Status = TransitivelyReachable
				result.MinDistance = 2 // Conservative estimate: entry point -> vulnerable function

				// If there are multiple entry points calling different user functions, it's highly reachable
	...
	if e.verbose && result.Status != NotReachable {
		fmt.Printf("DEBUG: Cluster-based analysis found %d reachable paths\n", len(result.Paths))
	}
