...
func TestGetMethodRisk(t *testing.T) {
	...
	tests := []struct {
		...
		// High risk methods
		{"reflect.Call", RiskHigh},
		{"Value.Call", RiskHigh},
		{"Value.CallSlice", RiskHigh},
		{"reflect.MakeFunc", RiskHigh},
		{"Value.Set", RiskHigh},
		{"Value.SetInt", RiskHigh},

		// Medium risk methods
		{"reflect.MethodByName", RiskMedium},
		{"Value.MethodByName", RiskMedium},
		{"reflect.FieldByName", RiskMedium},
		{"Value.FieldByName", RiskMedium},
		{"Value.Elem", RiskMedium},

		// Low risk methods (default)
		{"reflect.TypeOf", RiskLow},
		{"reflect.ValueOf", RiskLow},
		{"Value.Type", RiskLow},
		{"unknown.method", RiskLow},
	}
...
`
	...
	for funcName, usage := range usageMap {
		if strings.Contains(funcName, "TestFunction") {
			...
			if usage.ReflectionRisk == RiskNone {
...
func TestGetSummary(t *testing.T) {
	usageMap := map[string]*Usage{
		"func1": {ReflectionRisk: RiskLow},
		"func2": {ReflectionRisk: RiskMedium},
		"func3": {ReflectionRisk: RiskHigh},
		"func4": {ReflectionRisk: RiskLow},
		"func5": {ReflectionRisk: RiskNone},
	}

	summary := GetSummary(usageMap)

	expected := map[RiskLevel]int{
		RiskNone:   1,
		RiskLow:    2,
		RiskMedium: 1,
		RiskHigh:   1,
...
`
	...
	// Verify results
	expectedFunctions := map[string]RiskLevel{
		"testpkg.HighRiskFunction":            RiskHigh,
		"testpkg.MediumRiskFunction":          RiskMedium,
		"testpkg.LowRiskFunction":             RiskLow,
		"testpkg.(TestType).ReflectiveMethod": RiskMedium,
	}
