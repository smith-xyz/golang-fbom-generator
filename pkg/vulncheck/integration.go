...
func (i *Integration) ScanAndConvert(packagePath, specificPackage string) (*cve.CVEDatabase, error) {
...
func (i *Integration) MergeWithExistingCVEFile(existingFile string, liveCVEData *cve.CVEDatabase) (*cve.CVEDatabase, error) {
	if i.verbose {
		fmt.Fprintf(os.Stderr, "Merging existing CVE file with live scan results\n")
	}

	existingLoader := cve.NewLoader(i.verbose)
...
func (i *Integration) CreateTempCVEFileFromDatabase(cveDB *cve.CVEDatabase) (string, func(), error) {
...
func (i *Integration) convertCVEDatabaseToScanResults(cveDB *cve.CVEDatabase) *ScanResults {
	results := &ScanResults{
...
func (i *Integration) writeCVEDatabaseToFile(cveDB *cve.CVEDatabase, file *os.File) error {
	if cveDB == nil {
		cveDB = &cve.CVEDatabase{CVEs: []cve.CVE{}}
	}
