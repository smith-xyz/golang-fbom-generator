...
// TestUnifiedFBOMGeneration tests the unified FBOM generation approach
func TestUnifiedFBOMGeneration(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			expectError: false,
			description: "Should generate FBOM for external dependency",
		},
		{
			name:        "Standard library package",
			packageSpec: "fmt",
			expectError: false,
			description: "Should generate FBOM for stdlib package",
		},
		{
			name:        "Standard library package with version",
			packageSpec: "encoding/json@go1.21.0",
			expectError: false,
			description: "Should generate FBOM for stdlib package with specific version",
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := GenerateFBOM(tt.packageSpec, "", false, "rta", nil)
...
func TestUnifiedFBOMGenerationWithOptions(t *testing.T) {
	tests := []struct {
		name        string
		packageSpec string
		verbose     bool
		...
		},
		{
			...
			entryPoints: nil,
			expectError: false,
			description: "Should generate FBOM for external package with VTA",
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			err := GenerateFBOM(tt.packageSpec, "", tt.verbose, tt.algorithm, tt.entryPoints)
