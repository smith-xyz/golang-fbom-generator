...
func TestCVEGetSeverity(t *testing.T) {
	tests := []struct {
		name     string
		severity string
		expected Severity
	}{
		{"low severity", "Low", SeverityLow},
		{"medium severity", "Medium", SeverityMedium},
		{"high severity", "High", SeverityHigh},
		{"critical severity", "Critical", SeverityCritical},
		{"lowercase critical", "critical", SeverityCritical},
		{"unknown severity", "Unknown", SeverityLow}, // defaults to low
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			cve := CVE{OriginalSeverity: tt.severity}
			result := cve.GetSeverity()
...
func TestParseSeverity(t *testing.T) {
	tests := []struct {
		...
		{"CRITICAL", SeverityCritical},
		{"Unknown", SeverityLow}, // defaults to low
		{"", SeverityLow},        // empty string defaults to low
	}
	...
	for _, tt := range tests {
		t.Run(tt.input, func(t *testing.T) {
			...
			if result != tt.expected {
				t.Errorf("ParseSeverity(%s) = %v, want %v", tt.input, result, tt.expected)
			}
		})
	}
}
...
func TestConvertJiraTicketsToCVEs(t *testing.T) {
	testTickets := []JiraTicket{
		{
			IssueKey:    "SEC-123",
	...
	cves := ConvertJiraTicketsToCVEs(testTickets)

	if len(cves) != len(testTickets) {
		t.Errorf("Expected %d CVEs, got %d", len(testTickets), len(cves))
	}

	// Check first CVE has JIRA metadata
	if len(cves) > 0 {
		firstCVE := cves[0]

		if firstCVE.Metadata == nil {
			t.Fatal("Expected metadata to be populated")
		}

		expectedJiraKey := testTickets[0].IssueKey
		if firstCVE.Metadata["jira_key"] != expectedJiraKey {
			t.Errorf("Expected jira_key %s, got %s", expectedJiraKey, firstCVE.Metadata["jira_key"])
		}

		expectedAssignee := testTickets[0].Assignee
		if firstCVE.Metadata["jira_assignee"] != expectedAssignee {
			t.Errorf("Expected jira_assignee %s, got %s", expectedAssignee, firstCVE.Metadata["jira_assignee"])
		}

		expectedDueDate := testTickets[0].DueDate
		if firstCVE.Metadata["jira_due_date"] != expectedDueDate {
			t.Errorf("Expected jira_due_date %s, got %s", expectedDueDate, firstCVE.Metadata["jira_due_date"])
		}
	}

	// Check that CVE data is preserved
	if len(cves) > 0 {
		if cves[0].ID != testTickets[0].CVEData.ID {
			t.Errorf("Expected CVE ID %s, got %s", testTickets[0].CVEData.ID, cves[0].ID)
		}
	}
}
...
func TestEmptyDatabase(t *testing.T) {
	...
	cves = db.FindBySeverity(SeverityHigh)
	if len(cves) != 0 {
...
func BenchmarkFindBySeverity(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		db.FindBySeverity(SeverityHigh)
	}
}
...
func BenchmarkParseSeverity(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		ParseSeverity(severities[i%len(severities)])
	}
}
