...
func (g *FBOMGenerator) buildUserFunctionInventory(reflectionUsage map[string]*reflection.Usage, callGraph *callgraph.Graph, ssaProgram *ssa.Program) []Function {
	...
	if callGraph != nil {
		g.logger.Debug("Processing call graph functions", "total_nodes", len(callGraph.Nodes))
		for fn := range callGraph.Nodes {
			...
			if !g.isUserDefinedFunction(fn) {
				if g.verbose {
					...
					g.logger.Debug("Skipping non-user function", "function", fn.Name(), "package", pkg)
				}
				continue
			}
...
// Rest of the methods are simplified versions of the original methods...
// I'll implement the key ones needed for basic functionality
