...
// TestEntryPointDetection tests the detection of additional entry points
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expectedSuccess && tt.additionalEntryPoints != nil {
				...
				if len(generator.additionalEntryPoints) != expectedCount {
					t.Errorf("Expected %d stored entry points, got %d", expectedCount, len(generator.additionalEntryPoints))
				}
			}
		})
	}
...
// TestEntryPointPatternMatching tests pattern matching for entry point identification
func TestEntryPointPatternMatching(t *testing.T) {
	generator := NewFBOMGenerator(true)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			matches := generator.matchesEntryPointPattern(tt.functionName, tt.patterns)
