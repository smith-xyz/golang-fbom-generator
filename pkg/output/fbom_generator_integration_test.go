...
`
	...
	for _, fn := range fbom.Functions {
		...
		if generator.isStandardLibraryPackage(fn.Package) {
		...
		if generator.isDependencyPackage(fn.Package) {
...
// Helper function to build a call graph from Go source code
...
`
	...
	return graph.CallGraph, prog, nil
}
...
`
	...
	generator := NewFBOMGenerator(false) // Non-verbose for benchmarking
